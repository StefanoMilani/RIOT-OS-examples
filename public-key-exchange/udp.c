...
int handle_message(char* src_addr) {
    char str_msg_type;
    uint8_t msg_type;
    uint8_t* src_pub = (uint8_t*) malloc(sizeof(uint8_t)*(public_key_size));
    // Get message type from message (First char)
    memcpy(&str_msg_type, server_buffer, MSG_TYPE_SIZE);
    msg_type = atoi(&str_msg_type);
    // Get compressed key from message
    for(size_t i = 0 ; i < public_key_size ; i++) {
        src_pub[i] = (uint8_t) server_buffer[MSG_TYPE_SIZE + i];
    }
    switch (msg_type) {
        case KEY_REQUEST:
            printf("\n-PUBLIC KEY REQUEST FROM %s\n", src_addr);
            printf("Public key received:");
            print_key(src_pub, public_key_size);
            // Store info of struct
            store_dev_info(src_addr, src_pub);
            // Clean server buffer for response
            memset(server_buffer, 0, SERVER_BUFFER_SIZE);
            // Fill server_buffer with your key
            sprintf(server_buffer, "%u", (char)KEY_ACK);
        ...
        case KEY_ACK:
            ...
            sprintf(server_buffer, "%u", (char)ACK);
            // Send ACK
            printf("\n-SENDING ACK TO %s.\n", src_addr);
        ...
        case MSG:
            ...
            // Set key
            tc_aes128_set_decrypt_key(&s, (const uint8_t *)dev->secret);
            // Decrypt
            uint8_t msg[TC_AES_BLOCK_SIZE + 1];
            tc_aes_decrypt(msg, src_pub, &s);
...
int start_exchange(int argc, char **argv){
    ...
    sprintf(buf, "%u", (uint8_t)KEY_REQUEST);
...
int send_encrypted(int argc, char **argv){
    ...
    // Initialize key
    tc_aes128_set_encrypt_key(&s, dev->secret);

    // Encrypt data
    uint8_t cipher[TC_AES_BLOCK_SIZE + 1];
    tc_aes_encrypt(cipher, (const uint8_t *)argv[2], &s);

    // Forge server buffer
    sprintf(server_buffer, "%u", (uint8_t)MSG);
    memcpy(server_buffer+MSG_TYPE_SIZE, cipher, TC_AES_BLOCK_SIZE + 1);
    
    // Send encrypted message
    udp_send(argv[1], DEFAULT_PORT, server_buffer, TC_AES_BLOCK_SIZE + 1 + MSG_TYPE_SIZE, DEFAULT_NUM, DEFAULT_DELAY);

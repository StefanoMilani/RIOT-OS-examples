...
int decrypt_received_message(char* src_addr);
...
int handle_message(char* src_addr) {
    ...
    switch (msg_type) {
        ...
        case MSG:
			...
			if(decrypt_received_message(src_addr) == -1) {
				perror("Failed to decrypt received message");
				return -1;
			}
            break;
        default:
...
int decrypt_received_message(char* src_addr) {
    ...
    // Set key
    if(tc_aes128_set_decrypt_key(&s, (const uint8_t *)dev->secret) == TC_CRYPTO_FAIL) {
		perror("Failed to set aes decrypt key");
		return -1;
	} 
    // Decrypt
    uint8_t msg[TC_AES_BLOCK_SIZE + 1];
    if(tc_aes_decrypt(msg, (const uint8_t*) server_buffer+MSG_TYPE_SIZE, &s) == TC_CRYPTO_FAIL) {
